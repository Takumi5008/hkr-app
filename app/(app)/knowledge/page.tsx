    ...
    <div className="p-4 sm:p-6 max-w-3xl mx-auto">
      ...
      {canUpload && (
        <button onClick={openUpload}
          ...
          <Plus size={15} />{tab === 'minutes' ? '議事録を投稿' : tab === 'course' ? '極意書を投稿' : 'ファイルをアップロード'}
        </button>
      )}
      ...
      {loading ? (
        ...
        return filtered.length === 0 ? (
          <div className="text-center py-16 text-gray-400">
            ...
            {canUpload && <p className="text-xs mt-1">{tab === 'minutes' ? '「議事録を投稿」ボタンから追加できます' : tab === 'course' ? '「極意書を投稿」ボタンから追加できます' : '「アップロード」ボタンから追加できます'}</p>}
          </div>
        ) : (
              ...
              <div key={m.id} className="bg-white rounded-2xl border border-gray-200 p-4 shadow-sm">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex items-center gap-1 shrink-0 relative z-10">
                    ...
                    {m.url && (
                      <button onClick={() => handleDownload(m.url)}
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h2 className="text-base font-bold text-gray-900">
                {tab === 'minutes' ? '議事録を投稿' : tab === 'course' ? '極意書を投稿' : isTextMode ? 'テキストを投稿' : 'ファイルをアップロード'}
              </h2>
              ...
              {isTextMode ? (
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">
                    {tab === 'minutes' ? '議事録内容' : tab === 'course' ? '極意書の内容' : 'テキスト'} <span className="text-red-400">*</span>
                  </label>
                  <textarea
                    value={form.content}
