...
const MEMBER_SECTIONS: Section[] = [
  ...
  },
  {
    ...
    items: [
      { label: '実績を入力する', desc: '商材ごとに「解除数」と「開通数」を入力して保存します。入力した月は画面上部のセレクターで切り替えできます。' },
  ...
  },
  {
    ...
    items: [
      ...
      { label: '締切リマインダー', desc: '締切1時間前に未入力の場合、プッシュ通知が届きます。' },
    ],
  },
  {
    title: 'タスク管理',
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'インフラ講座タブ', desc: 'インフラ講座の動画・PDF・音声ファイルのアップロードに加え、「テキスト直接入力」を選ぶとテキストをそのまま貼り付けて投稿できます。動画はインラインプレイヤーで再生可能。テキスト投稿は▼ボタンで全文展開して読めます。' },
...
const MANAGER_EXTRA_SECTIONS: Section[] = [
  {
    ...
    color: 'bg-violet-50 border-violet-200',
    items: [
      { label: '個人タブ', desc: '名前ボタンを切り替えてメンバーごとの年別月次データ（獲得数・開通数・解除数・稼働日数・稼働時間）を確認できます。各月の鉛筆アイコンで数値を編集できます。' },
  ...
  },
  {
    ...
    color: 'bg-rose-50 border-rose-200',
    items: [
      { label: '出欠の確認', desc: '管理ページ →「MTG管理」タブで全員の出欠状況を一覧で確認できます。未入力のメンバーがいる場合は上部に未提出者の確認バナーが表示されます。' },
  ...
  },
  {
    ...
    items: [
      ...
      { label: '反映ルール（共通）', desc: '解除⭕️かつ工事日・受取日・開通予定日が確定している案件のみ開通カレンダーに表示されます。「未定」「-」の日付は表示されません。入力した月ではなく、実際の工事日・受取日が属する月のカレンダーに反映されます。' },
      { label: 'WiMAX後送りの反映ルール', desc: '2026年5月以前の案件：獲得日がカレンダーに表示されます。2026年6月以降の案件：獲得日＋7日がカレンダーに表示されます。受取⭕️が付いた時点で反映されます。' },
