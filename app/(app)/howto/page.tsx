...
const MEMBER_SECTIONS: Section[] = [
  {
    ...
    items: [
      { label: '今日やること', desc: '商材ごとのHKR入力状況をチェックリストで表示します。入力済みの商材はチェック済みで表示され、未入力の場合は「入力する」リンクが表示されます。本日フォロー対応が必要なお客様がいる場合もここに表示されます。' },
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'アップロード（管理者・マネージャーのみ）', desc: '「アップロード」ボタンからタイトル・説明・ファイルを選択してアップロードできます。対応形式：音声（MP3/WAV/M4A）、動画（MP4/MOV）、PDF、テキスト。' },
    ],
  },
  {
    title: 'ミッション',
