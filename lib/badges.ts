...
export const BADGES: BadgeDef[] = [
  ...
  { id: 'monthly_open_7',  name: '月7開通',           icon: '🌟', desc: '1ヶ月に7開通を達成',           color: 'bg-sky-100 text-sky-700 border-sky-200' },
  { id: 'monthly_open_10', name: '月10開通',          icon: '💎', desc: '1ヶ月に10開通を達成',          color: 'bg-blue-100 text-blue-700 border-blue-200' },
  { id: 'monthly_open_15', name: '月15開通',          icon: '🏅', desc: '1ヶ月に15開通を達成',          color: 'bg-violet-100 text-violet-700 border-violet-200' },
  { id: 'monthly_open_20', name: '月20開通',          icon: '👑', desc: '1ヶ月に20開通を達成',          color: 'bg-amber-100 text-amber-700 border-amber-200' },
]
...
export async function awardBadges(userId: number): Promise<string[]> {
  ...
  // 月ごとの最大開通数（opening_calendar status=○）
  const monthlyOpenRow = await dbQueryOne<{ max_cnt: number }>(
    `SELECT COALESCE(MAX(cnt), 0)::int AS max_cnt
     FROM (
  ...
  const maxMonthlyOpen = monthlyOpenRow?.max_cnt ?? 0
  ...
  const conditions: Record<string, boolean> = {
    ...
    monthly_open_7:   maxMonthlyOpen >= 7,
    monthly_open_10:  maxMonthlyOpen >= 10,
    monthly_open_15:  maxMonthlyOpen >= 15,
    monthly_open_20:  maxMonthlyOpen >= 20,
  }
